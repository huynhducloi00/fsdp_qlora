...
def fsdp_main(local_rank: int, world_size: int, args: Dict):
    ...
    if args["train_type"] in ["lora", "qlora"]:
        ...
        if args["train_type"] == "hqq_dora":
            print_0(rank, f"Using HQQDORA")
            lora_cls = HQQDORA
        elif args["train_type"] == "bnb_dora":
            print_0(rank, "Using BNB DORA")
            lora_cls = BNBDORA
        else:
            print_0(rank, "Using LORA")
            lora_cls = LORA
        # Create LORA layers.
    ...
    if args["save_model"]:
        ...
        if args["train_type"] in [
            ...
            for prefix, module in trainable_fsdp_modules:
                ...
                dist.barrier()
                torch.cuda.synchronize()
                print_0(rank,"Saving trained LoRA weights.")
        else:
            with FSDP.state_dict_type(
                model, StateDictType.FULL_STATE_DICT, save_policy
            ):
                cpu_state_dict = model.state_dict()
                print_0(rank,"Saving full model weights.")

        if rank == 0:
            parent_path =f"{args['output_dir']}/{args['model_name']}"
            os.makedirs(parent_path, exist_ok=True)

            save_file(
                cpu_state_dict,
                os.path.join(parent_path,f"{args['train_type']}_{args['dataset']}_{args['dataset_samples']}_dict.safetensors")
                )
            print("Done", rank)
...
def main(
    ...
    ) = "qlora",  # "full", "lora", "qlora", or "custom_qlora" # type: ignore
    ...
    output_dir: str = "models",  # Output directory to save the final model to
    ...
    optimizer: Param(
        "", choices=["adamw", "adam", "sgd", "adadelta", "fused_adam", "fused_adamw"]
    ) = "adamw",  # Optimizer. PyTorch 2.4 nightly adds CPU fused Adam/AdamW which should improve offload training speed.
    lr_scheduler: Param(
